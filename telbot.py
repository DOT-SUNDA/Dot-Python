...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'startall':
        ...
        await query.edit_message_text("➕ Kirim IP yang ingin ditambahkan (contoh: 192.168.1.1):")
        return ADD_IP
    elif data == 'clearip':
        save_ips([])
...
async def add_ip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not validate_ip(ip):
        await update.message.reply_text("⚠️ Format IP tidak valid. Contoh format yang benar: 192.168.1.1")
        return ADD_IP
...
if __name__ == '__main__':
    ...
    app.add_handler(CallbackQueryHandler(
        button_handler,
        pattern='^setlink$|^addip$|^remove_.*$|^startall$|^stopall$|^manageip$|^clearip$|^back$'
    ))
    
    # Handler untuk target selection
    app.add_handler(CallbackQueryHandler(
        lambda update, context: handle_start_all(update.callback_query),
    ...
    conv_handler = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(button_handler, pattern='^setlink$|^addip$')
        ],
        states={
